...
""",
    "help": {
        "en": """
<h2>How it works</h2>
<p>The diff is computed line-by-line using the Longest Common Subsequence algorithm — the same approach <code>diff</code> and Git use. Lines that match drop into the "unchanged" stream; everything else becomes an addition or removal.</p>
<h3>Whitespace and case</h3>
<p>The toggles affect comparison only — display still shows the original text. Useful when the only difference is reformatting or trailing spaces.</p>
<h3>Side-by-side vs unified</h3>
<ul>
  <li><strong>Side-by-side</strong> — easier to scan small changes per line.</li>
  <li><strong>Unified</strong> — closer to <code>git diff</code> output; better for sharing or printing.</li>
</ul>
""",
    },
